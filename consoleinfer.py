...
def run(key, input, model_path):
    ...
    model, vocab = get_pytorch_conkogpt2_model(model_path)
    ...
    topics = []
    for topic in key.split(","):
        topics += vocab[tokenizer(topic)]
    source = vocab[tokenizer(input)]
    out_ids = sample_sequence(topics, source, vocab, tokenizer, model, args)
    out_texts = vocab.to_tokens(out_ids)
    result = ""
    for text in out_texts:
        result += text.replace('▁', ' ').replace('</s>', ' ')
    print(result)
...
if __name__ == "__main__":
    run("김치찌개","저녁으로 무엇을 먹을까?", "/workspace/KoGPT2/runs/clm_1_12/checkpoint_mymodel_56450.pth")