...
def train():
    ...
    if args.distributed:
        gpt_model = DistributedDataParallel(gpt_model, device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    def update(engine, batch):
        ...
        (lm_loss), *_ = gpt_model(target_ids, encoded_layers[-1], labels=lm_labels)
        ...
        if args.fp16:
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(gpt_model.parameters(), args.max_norm)
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            batch = tuple(input_tensor.to(args.device) for input_tensor in batch)
            source_ids, target_ids, lm_labels = batch
            #logger.info(vocab.to_tokens(input_ids.squeeze(0).tolist()))
            # if we dont send labels to model, it doesnt return losses

            encoded_layers, pooled_output = bert_model(source_ids)
            #lm_logits, *_ = model(input_ids, token_type_ids=token_type_ids,)
            lm_logits, *_ = gpt_model(target_ids, encoded_layers[-1])
